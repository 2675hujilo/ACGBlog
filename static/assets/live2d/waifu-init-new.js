/* Bug11 文件头注释
 * 看板娘初始化（新版）：整合引擎加载、换装、提示与交互，供前端按需引用。
 */
/* ============================================================
   Live2D 看板娘初始化脚本（新引擎版 - 支持Cubism 2/3/4/5全格式）
   基于 pixi-live2d-display 渲染器
...
(function () {
    ...
    var gameModal = null;
    var waifuContainer = null;

    // 模型配置（支持Cubism2和Cubism3+）
    var MODEL_CONFIG = {
        'shizuku': { path: '/static/assets/live2d/models/shizuku/shizuku.model.json', name: '雫', skins: 6, type: 'cubism2' },
        'shizuku_talk': { path: '/static/assets/live2d/models/shizuku_talk/shizuku-48/index.json', name: '雫Talk', skins: 2, type: 'cubism2' },
        'koharu': { path: '/static/assets/live2d/models/koharu/koharu.model.json', name: '小春', skins: 1, type: 'cubism2' },
        'haru': { path: '/static/assets/live2d/models/haru/haru01.model.json', name: '春', skins: 3, type: 'cubism2' },
        'hijiki': { path: '/static/assets/live2d/models/hijiki/hijiki.model.json', name: '黑猫', skins: 1, type: 'cubism2' },
        'tororo': { path: '/static/assets/live2d/models/tororo/tororo.model.json', name: '白猫', skins: 1, type: 'cubism2' },
        'wanko': { path: '/static/assets/live2d/models/wanko/wanko.model.json', name: '狗狗', skins: 1, type: 'cubism2' },
        'hibiki': { path: '/static/assets/live2d/models/hibiki/hibiki.model.json', name: '响', skins: 1, type: 'cubism2' },
        'izumi': { path: '/static/assets/live2d/models/izumi/izumi.model.json', name: '泉', skins: 1, type: 'cubism2' },
        'tsumiki': { path: '/static/assets/live2d/models/tsumiki/tsumiki.model.json', name: '积木', skins: 1, type: 'cubism2' },
        'chitose': { path: '/static/assets/live2d/models/chitose/chitose.model.json', name: '千岁', skins: 1, type: 'cubism2' },
        'haruto': { path: '/static/assets/live2d/models/haruto/haruto.model.json', name: '春人', skins: 1, type: 'cubism2' },
        'miku': { path: '/static/assets/live2d/models/miku/miku.model.json', name: '初音', skins: 1, type: 'cubism2' },
        'ni-j': { path: '/static/assets/live2d/models/ni-j/ni-j.model.json', name: '仁藤', skins: 1, type: 'cubism2' },
        'nico': { path: '/static/assets/live2d/models/nico/nico.model.json', name: '妮可', skins: 1, type: 'cubism2' },
        'nipsilon': { path: '/static/assets/live2d/models/nipsilon/nipsilon.model.json', name: '尼普西隆', skins: 1, type: 'cubism2' },
        'nito': { path: '/static/assets/live2d/models/nito/nito.model.json', name: '仁斗', skins: 1, type: 'cubism2' },
        'unitychan': { path: '/static/assets/live2d/models/unitychan/unitychan.model.json', name: 'Unity娘', skins: 1, type: 'cubism2' },
        'z16': { path: '/static/assets/live2d/models/z16/z16.model.json', name: '小可爱', skins: 1, type: 'cubism2' },
    ...
    function loadModel(modelUrl, callback) {
        if (typeof Live2DRenderer === 'undefined' || !Live2DRenderer.loadModel) {
            st('Live2D新引擎未加载喵~', 3000);
            return;
        }
        try {
            console.log('[Waifu] loadModel (new engine):', modelUrl);
            Live2DRenderer.loadModel('live2d-canvas', modelUrl, function(err, model) {
                if (err) {
                    console.error('[Waifu] loadModel error:', err);
                    st('模型加载出错: ' + err.message, 5000);
                } else {
                    console.log('[Waifu] model loaded successfully');
                    if (callback) callback();
                }
            });
        } catch (e) {
            console.error('[Waifu] loadModel exception:', e);
    ...
    function switchModel() {
        var currentIdx = MODEL_IDS.indexOf(state.modelId);
        var nextIdx = (currentIdx + 1) % MODEL_IDS.length;
        var nextId = MODEL_IDS[nextIdx];
        var next = MODEL_CONFIG[nextId];
    ...
    // 随机切换模型
    function randModel() {
        var available = MODEL_IDS.filter(function(id) { return id !== state.modelId; });
    ...
    // 切换皮肤（顺序）
    function switchSkin() {
        var cfg = SKIN_CONFIGS[state.modelId];
        if (!cfg || cfg.count <= 1) {
            st('这个角色只有一套衣服喵~', 3000);
            return;
        }
    ...
    function takePhoto() {
        try {
            var model = Live2DRenderer.getModel();
            if (!model) {
                st('找不到模型喵~', 3000);
                return;
            }
            // 使用PIXI渲染器提取canvas
            var app = Live2DRenderer.getApp ? Live2DRenderer.getApp() : null;
            if (app && app.renderer) {
    ...
    function init() {
        console.log('[Waifu] init start (new engine)');
        ...
        w.innerHTML =
            ...
            '<div id="live2d-canvas-container" style="width:220px;height:250px;position:absolute;bottom:0;left:0;cursor:pointer;">' +
        ...
        // 加载初始模型
        var initConfig = MODEL_CONFIG[state.modelId] || MODEL_CONFIG['shizuku'];
        console.log('[Waifu] init modelId:', state.modelId, 'skin:', state.skin, 'url:', initConfig.path);
        loadModel(initConfig.path, function () {
        ...
        // 点击交互
        var canvasContainer = document.getElementById('live2d-canvas-container');
        if (canvasContainer) {
